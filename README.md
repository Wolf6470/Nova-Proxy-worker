...
این بخش برای تنظیم روش‌های مختلف اتصال پروکسی است. اگه خالی بذاری، از حالت پیش‌فرض (بدون پروکسی اضافی) استفاده میشه.

| آیتم | توضیح | فرمت | مثال |
|------|-------|------|------|
| **اشتراک برگزیده (sub)** | آدرس دامنه‌ای که اشتراک از اون سرو میشه | دامنه | `sub.example.com` |
| **پراکسی‌آی‌پی** | آدرس IP پروکسی برای تغییر IP خروجی | `host:port` | `proxyip.example.com:443` |
| **SOCKS5** | پروکسی SOCKS5 با یوزر و پسورد | `user:pass@host:port` | `user:pass@127.0.0.1:1080` |
...
- برای استفاده از پروکسی آماده، باید خودت یه سرور پروکسی داشته باشی یا از سرویس‌های رایگان استفاده کنی
